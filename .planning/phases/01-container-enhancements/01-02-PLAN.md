...
must_haves:
  truths:
    - "HUD container checks for rehydration on every service open"
    - "HUD container shows 'Restoring data from server...' during rehydration"
    - "HUD container displays 'HUD #1', 'HUD #2' when multiple HUD services exist"
    - "Breadcrumbs include instance number when totalHUDServices > 1"
  artifacts:
