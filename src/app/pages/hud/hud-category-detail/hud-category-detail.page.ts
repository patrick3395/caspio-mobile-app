...
export class HudCategoryDetailPage implements OnInit, OnDestroy, ViewWillEnter, HasUnsavedChanges {
    ...
    try {
      ...
      if (environment.isWeb) {
        ...
        for (const attach of attachments) {
          ...
          let displayUrl = attach.Attachment || attach.Photo || attach.url || 'assets/img/photo-placeholder.svg';
  ...
  async addPhotoFromCamera(category: string, itemId: string | number) {
    ...
    // Get or create HUD record first
    let visualId = this.visualRecordIds[key];
    if (!visualId) {
    ...
    if (!visualId) {
      console.error('[HUD CAMERA] Failed to create HUD record');
    ...
    const config: PhotoCaptureConfig = {
      ...
      onUploadFailed: (tempId: string, error: any) => {
        console.error('[HUD CAMERA] Upload failed:', error);
        const index = this.visualPhotos[key].findIndex(p =>
          p.AttachID === tempId || p.imageId === tempId
        );
        if (index !== -1) {
  ...
  async addPhotoFromGallery(category: string, itemId: string | number) {
    ...
    if (!visualId) {
      console.error('[HUD GALLERY] Failed to create HUD record');
      await this.showToast('Failed to create record for photo', 'danger');
      return;
    }
    ...
    const config: PhotoCaptureConfig = {
      ...
      onUploadFailed: (tempId: string, error: any) => {
        console.error('[HUD GALLERY] Upload failed:', error);
        const index = this.visualPhotos[key].findIndex(p =>
          p.AttachID === tempId || p.imageId === tempId
        );
        if (index !== -1) {
  ...
  // All old camera/gallery code removed - now using PhotoHandlerService
  ...
  openVisualDetail(categoryName: string, item: VisualItem) {
    ...
    if (environment.isWeb) {
      ...
      // Custom visual: id = "custom_12345" -> extract HUDID directly
