...
export class HudMainPage implements OnInit {
  ...
  private async checkCompletionStatus() {
    ...
    console.log('[HUD Main] Checking completion status...');
    ...
    try {
      ...
      const sectionMap: { [key: string]: string } = {
        ...
        'HUD / Manufactured Home': 'HUD / Manufactured Home'
      };
      ...
      this.cards.forEach(card => {
        ...
        if (mappedSection) {
          ...
          console.log(`[HUD Main] ${card.title}: ${card.completed ? 'Complete' : `Incomplete (${incompleteCount} fields)`}`);
        }
      });
    } catch (error) {
      console.error('[HUD Main] Error checking completion status:', error);
    }
  }
