...
export class LbwCategoryDetailPage implements OnInit, OnDestroy, ViewWillEnter {
  projectId: string = '';
  serviceId: string = '';
  categoryName: string = '';
   ...
   * WEBAPP FIX: Use LBWID-first matching to prevent mismatch when titles are edited
   */
  private async processVisualsUpdate(categoryVisuals: any[]): Promise<void> {
    ...
    for (const visual of categoryVisuals) {
      ...
      // WEBAPP FIX: PRIORITY 1 - Find item by LBWID using existing visualRecordIds mapping
      ...
      for (const [key, storedLbwId] of Object.entries(this.visualRecordIds)) {
        if (String(storedLbwId) === LBWID) {
      ...
      // PRIORITY 2: Fall back to name matching if no existing LBWID mapping
      if (!item) {
        item = allItems.find(i => i.name === visual.Name);
        if (item) {
        }
  ...
  isItemSelected(category: string, itemId: string | number): boolean {
    // WEBAPP FIX: Use itemId directly (which is item.id) - matches selectedItems storage key pattern
    const item = this.findItemById(itemId);
    const key = environment.isWeb
      ? `${category}_${itemId}`
    ...
    if (item && item.category && item.category !== category) {
      const itemCategoryKey = environment.isWeb
        ? `${item.category}_${itemId}`
        : `${item.category}_${item.templateId}`;
  ...
  isItemSaving(category: string, itemId: string | number): boolean {
    // WEBAPP FIX: Use itemId directly (which is item.id) - matches savingItems storage key pattern
    const item = this.findItemById(itemId);
    const key = environment.isWeb
      ? `${category}_${itemId}`
      : `${category}_${item?.templateId ?? itemId}`;
  ...
  getPhotosForVisual(category: string, itemId: string | number): any[] {
    // WEBAPP FIX: Use itemId directly (which is item.id) - photos are stored with ${category}_${item.id} key
    // In loadExistingVisuals and loadPhotosFromAPI, keys use item.id not templateId
    if (environment.isWeb) {
      const key = `${category}_${itemId}`;
      return this.visualPhotos[key] || [];
    }

    // MOBILE: Use item.templateId for key to match Dexie lookup pattern
    const item = this.findItemById(itemId);
  ...
  // ============================================
  // CAMERA AND GALLERY CAPTURE METHODS (CONSOLIDATED VIA PhotoHandlerService)
  // ============================================

  /**
   * Get the key for photo storage based on platform
   * WEBAPP uses itemId, MOBILE uses templateId
   */
  ...
  async addPhotoFromCamera(category: string, itemId: string | number) {
    ...
    try {
      const config: PhotoCaptureConfig = {
        ...
        onTempPhotoAdded: (photo: StandardPhotoEntry) => {
          ...
          const existingIndex = this.visualPhotos[key].findIndex((p: any) =>
            p.imageId === photo.imageId || p.AttachID === photo.imageId
          );
          if (existingIndex === -1) {
        ...
        onUploadComplete: (photo: StandardPhotoEntry, tempId: string) => {
          // Replace temp photo with final photo
          const tempIndex = this.visualPhotos[key].findIndex((p: any) => p.imageId === tempId);
          if (tempIndex >= 0) {
            this.visualPhotos[key][tempIndex] = photo;
          }
      ...
      // RACE CONDITION FIX: Re-enable liveQuery
  ...
  async addPhotoFromGallery(category: string, itemId: string | number) {
    ...
    if (!isTempId && isNaN(visualIdNum) && environment.isWeb) {
      console.error('[GALLERY UPLOAD] Invalid LBW ID:', visualId);
      return;
    }
    ...
    try {
      const config: PhotoCaptureConfig = {
        ...
        onUploadComplete: (photo: StandardPhotoEntry, tempId: string) => {
          // Replace skeleton/temp with final photo
          const tempIndex = this.visualPhotos[key].findIndex((p: any) =>
  ...
  // NOTE: This legacy method is kept for backwards compatibility but is no longer
  // used by the new PhotoHandlerService flow
  ...
  openVisualDetail(category: string, item: any): void {
    ...
    if (environment.isWeb) {
      ...
      // Custom visual: id = "custom_12345" -> extract LBWID directly
