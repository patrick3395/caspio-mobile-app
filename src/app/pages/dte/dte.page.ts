...
@Component({
  selector: 'app-dte',
  templateUrl: './dte.page.html',
  styleUrls: ['./dte.page.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule, IonicModule],
  changeDetection: ChangeDetectionStrategy.OnPush  // PERFORMANCE: OnPush for optimized change detection
})
export class DtePage implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Categories from Services_DTE_Templates
  ...
  // Track visual record IDs from Services_DTE table
  ...
  private async initializeOperationsQueue(): Promise<void> {
    ...
    this.operationsQueue.setExecutor('CREATE_VISUAL', async (data: any) => {
      ...
      const visualId = response.DTEID || response.PK_ID || response.id;
      if (!visualId) {
        console.error('[OperationsQueue] No DTEID in response:', response);
    ...
    this.operationsQueue.setExecutor('UPLOAD_VISUAL_PHOTO', async (data: any, onProgress?: (p: number) => void) => {
      ...
      const response = await this.caspioService.createServicesDTEAttachWithFile(
    ...
    this.operationsQueue.setExecutor('UPLOAD_VISUAL_PHOTO_UPDATE', async (data: any, onProgress?: (p: number) => void) => {
      ...
      const response = await this.caspioService.updateServicesDTEAttachPhoto(
        data.attachId,
        compressedFile,
        data.originalFile
      ).toPromise();
  ...
  // Load dropdown options for visual templates from LPS_Services_DTE_Drop table
  ...
  async loadExistingData() {
    ...
    // Load existing visual selections from Services_DTE table
  ...
  async toggleItemSelection(category: string, itemId: string) {
    ...
    try {
      // Save or remove from Services_DTE table
      if (this.selectedItems[key]) {
        // Item was selected - save to Services_DTE
        await this.saveVisualSelection(category, itemId);
        // Success toast removed per user request
      } else {
        // Item was deselected - remove from Services_DTE if exists
  ...
  // Save visual selection to Services_DTE table
    ...
    try {
      ...
      try {
        ...
        if (existingVisuals) {
          ...
          if (exists) {
            const existingId = exists.DTEID || exists.PK_ID || exists.id;
      ...
      const visualData: ServicesDteRecord = {
  ...
  // Remove visual selection from Services_DTE table
      ...
      message: `
        <div style="text-align: left; font-family: monospace; font-size: 12px;">
          ...
          ${this.visualRecordIds[key] ? 
            ...
            (answersText ? 'Ã¢Å¾â€¢ CREATE new Services_DTE record' : 'Ã¢Å¡Â Ã¯Â¸Â No action - no selections')}<br>
        </div>
      `,
      buttons: ['Continue'],
      cssClass: 'wide-alert'
      ...
      if (item.selectedOptions && item.selectedOptions.length > 0) {
        if (existingVisualId) {
          ...
          const createAlert = await this.alertController.create({
            ...
            message: `
              <div style="font-family: monospace; font-size: 12px;">
                <strong style="color: blue;">Ã¢Å¾â€¢ CREATING Services_DTE</strong><br><br>
  ...
  async showFullText(item: any) {
    ...
    if (item.answerType === 1) {
      ...
      // Dropdown from Services_DTE_Drop
      const options = this.visualDropdownOptions[item.templateId] || [];
      if (options.length > 0) {
  ...
  async uploadPhotoForVisual(visualId: string, photo: File, key: string, isBatchUpload: boolean = false, annotationData: any = null, originalPhoto: File | null = null, caption: string = '') {
    ...
    try {
      ...
      if (isNaN(visualIdNum)) {
        throw new Error(`Invalid DTEID: ${actualVisualId}`);
      }
      ...
      const dataToSend = {
        ...
        process: [
          ...
          '2. Create record with DTEID and Annotation (without Photo)',
      ...
      if (false && !isBatchUpload) {
        const alert = await this.alertController.create({
          header: 'Services_DTE_Attach Upload Debug',
        message: `
          <div style="text-align: left; font-family: monospace; font-size: 12px;">
            ...
            Ã¢â‚¬Â¢ DTEID: <strong style="color: red;">${dataToSend.fields.DTEID}</strong> (Integer)<br>
  ...
  private async performVisualPhotoUpload(
    ...
    try {
      ...
      let response;
      try {
        response = await this.caspioService.createServicesDTEAttachRecord(
          visualIdNum,
          caption || '',
          drawingsData
        ).toPromise();
  ...
  async createCustomVisual(category: string, kind: string, name: string, text: string) {
    try {
      ...
      try {
        ...
        const debugAlert = await this.alertController.create({
          ...
          message: `
            <div style="text-align: left; font-family: monospace; font-size: 12px;">
              ...
              Ã¢â‚¬Â¢ DTEID (PRIMARY): <strong style="color: green;">${response?.DTEID || 'NOT FOUND'}</strong><br>
              ...
              Ã¢â‚¬Â¢ Key: ${category}_${response?.DTEID || response?.PK_ID || Date.now()}<br>
              Ã¢â‚¬Â¢ DTEID for photos: <strong style="color: green;">${response?.DTEID || response?.PK_ID || 'MISSING!'}</strong>
            </div>
          `,
        ...
        // Use DTEID from response, NOT PK_ID
        const visualId = response?.DTEID || response?.PK_ID || Date.now().toString();
        const customItem = {
        ...
        // Store the visual ID for photo uploads - use DTEID from response!
        const key = `${category}_${customItem.id}`;
        this.visualRecordIds[key] = String(response?.DTEID || response?.PK_ID || customItem.id);
...
Stack: ${error?.stack}`;
  ...
  // Save caption to the Annotation field in Services_DTE_Attach table
  async saveCaption(photo: any, category: string, itemId: string) {
    try {
      ...
      // Update the Services_DTE_Attach record with the new caption
      const updateData = {
        Annotation: photo.caption || ''  // Save caption or empty string
      };

      await this.caspioService.updateServicesDTEAttach(photo.AttachID, updateData).toPromise();
  ...
  async showVisualCreationDebug(category: string, templateId: string, response: any) {
    ...
    if (response === undefined || response === null || response === '') {
      ...
      visualIdFromResponse = response[0].DTEID || 'Not found';
      pkId = response[0].PK_ID || 'Not found';
      extractedId = response[0].DTEID || response[0].PK_ID || response[0].id || 'Not found in array';
    } else if (response && typeof response === 'object') {
      if (response.Result && Array.isArray(response.Result) && response.Result.length > 0) {
        responseType = 'Object with Result array';
        visualIdFromResponse = response.Result[0].DTEID || 'Not found';
        pkId = response.Result[0].PK_ID || 'Not found';
        extractedId = response.Result[0].DTEID || response.Result[0].PK_ID || response.Result[0].id || 'Not found in Result';
      } else {
        responseType = 'Direct Object';
        visualIdFromResponse = response.DTEID || 'Not found';
        pkId = response.PK_ID || 'Not found';
        extractedId = response.DTEID || response.PK_ID || response.id || 'Not found in object';
      }
    } else {
  ...
  async refreshVisualId(category: string, templateId: string) {
    try {
      ...
      if (visuals && Array.isArray(visuals)) {
        ...
        if (ourVisual) {
          const visualId = ourVisual.DTEID || ourVisual.PK_ID || ourVisual.id;
          const recordKey = `visual_${category}_${templateId}`;
  ...
  async prepareStructuralSystemsData() {
    ...
    for (const category of categoryOrder) {
      ...
      for (const visual of categoryData.comments) {
        const DTEID = visual.DTEID || visual.PK_ID;
    ...
    try {
      ...
      // Fetch all Services_DTE records for this service
      const visuals = await this.hudData.getVisualsByService(this.serviceId);
      ...
      const attachmentPromises = visuals
        .filter(visual => visual.DTEID)
        .map(visual =>
          this.hudData.getVisualAttachments(visual.DTEID)
            .then(attachments => ({ visualId: visual.DTEID, attachments }))
            .catch(error => {
              console.error(`Error fetching attachments for visual ${visual.DTEID}:`, error);
              return { visualId: visual.DTEID, attachments: [] };
            })
        );
  ...
  private updateVisualInOrganizedData(visual: any): void {
    ...
    if (kind === 'comment' && this.organizedData[category].comments) {
      const existing = this.organizedData[category].comments.find((c: any) => c.DTEID === visual.DTEID);
      if (existing) {
      ...
      const existing = this.organizedData[category].limitations.find((l: any) => l.DTEID === visual.DTEID);
      if (existing) {
        existing.Text = visual.Text || existing.Text;
        existing.Notes = visual.Notes || existing.Notes;
        found = true;
      ...
      const existing = this.organizedData[category].deficiencies.find((d: any) => d.DTEID === visual.DTEID);
      if (existing) {
        existing.Text = visual.Text || existing.Text;
        existing.Notes = visual.Notes || existing.Notes;
        found = true;
    ...
    if (!found && visual.DTEID) {
      const key = `${category}-${kind}-${visual.DTEID}`;
