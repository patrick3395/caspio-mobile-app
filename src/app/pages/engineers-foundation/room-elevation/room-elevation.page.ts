...
export class RoomElevationPage implements OnInit, OnDestroy {
  ...
  /**
   * Handle notes input for auto-capitalization on desktop browsers
   */
  handleNotesInput(event: any) {
    ...
    // Check if we just typed a space after 'i'
    if (textBeforeCursor.length >= 2) {
      const lastTwoChars = textBeforeCursor.slice(-2);
      
      // Check if it's " i" (space followed by i)
      if (lastTwoChars === ' i' || (textBeforeCursor.length === 1 && lastTwoChars.charAt(0) === 'i')) {
        // Replace standalone 'i' with 'I'
        const beforeI = currentValue.substring(0, cursorPosition - 1);
        const afterCursor = currentValue.substring(cursorPosition);
        
        // Check if 'i' is standalone (space before or start of text, and space/punctuation/end after)
        const charBeforeI = cursorPosition >= 2 ? currentValue.charAt(cursorPosition - 2) : '';
        const isStandalone = (cursorPosition === 1 || charBeforeI === ' ' || charBeforeI === '' || /[.!?]/.test(charBeforeI));
        
        if (isStandalone) {
          const newValue = beforeI + 'I' + afterCursor;
          this.roomData.notes = newValue;
    ...
    if (cursorPosition >= 3) {
      ...
      // Check for sentence endings followed by space and a letter
      const sentenceEndPattern = /[.!?]\s[a-z]$/;
      if (sentenceEndPattern.test(lastThreeChars)) {
        const beforeLetter = currentValue.substring(0, cursorPosition - 1);
        const letter = currentValue.charAt(cursorPosition - 1).toUpperCase();
        const afterCursor = currentValue.substring(cursorPosition);
        
        const newValue = beforeLetter + letter + afterCursor;
        this.roomData.notes = newValue;
    ...
    // Capitalize first letter if it's the start of the text
    if (cursorPosition === 1 && /^[a-z]$/.test(currentValue.charAt(0))) {
      const newValue = currentValue.charAt(0).toUpperCase() + currentValue.substring(1);
      this.roomData.notes = newValue;
