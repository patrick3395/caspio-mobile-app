...
// PDF generation is handled by TemplatePdfService
...
export class HudContainerPage implements OnInit, AfterViewInit, OnDestroy {
  ...
  get isGeneratingPDF(): boolean {
    return this.templatePdfService.isGenerating();
  }
  ...
  ensureButtonsEnabled() {
    // isGeneratingPDF is now managed by TemplatePdfService
  }
...
// Handle taking FDF photos (Top, Bottom, Threshold) - using file input like room points
  ...
  async generatePDF(event?: Event) {
    ...
    try {
      await this.templatePdfService.generatePDF(this.projectId, this.serviceId);
    } catch (error) {
