...
export class EfeFieldRepoService {
  ...
  async createPointRecordsForRoom(
    ...
    for (const point of existing.elevationPoints) {
      ...
      let tempPointId: string | null = null;

      try {
        // Queue for sync - this creates a pending record and returns placeholder with _tempId
        const response = await foundationDataService.createEFEPoint(pointData, tempEfeId);
        // Use the tempId generated by createEFEPoint (NOT our own)
        tempPointId = response?._tempId || response?.PointID || `temp_point_${now}_${point.pointNumber}`;
        ...
        // Fallback: generate our own temp ID so point is at least functional in Dexie
        tempPointId = `temp_point_${now}_${point.pointNumber}`;
      }
